...
import type { FileOperationResult, JsonImportResult, TeamSummary, TeamValidationResult } from "../../shared/contracts";
...
export function OperationsOverlay() {
  ...
  async function runImportOperation(operation:()=>Promise<JsonImportResult>,label:string):Promise<void>{ setBusy(true); try{ const result=await operation(); if(result.canceled){ setMessage("Operação cancelada."); return; } const imported=[result.importedBuilds?`${result.importedBuilds} build(s) criada(s)`:null,result.updatedBuilds?`${result.updatedBuilds} build(s) atualizada(s)`:null,result.importedTeams?`${result.importedTeams} equipe(s) criada(s)`:null,result.updatedTeams?`${result.updatedTeams} equipe(s) atualizada(s)`:null].filter(Boolean).join(", ") || "nenhum registro alterado"; setMessage(`${label} concluída: ${imported}. ${result.warnings.length?`Avisos: ${result.warnings.join(" | ")}`:""}`); setTeams(await window.gestorPoke.teams.list()); }catch(error){ setMessage(error instanceof Error?error.message:"Falha na operação."); }finally{ setBusy(false); } }
    ...
    {open?<div className="operations-backdrop" onMouseDown={()=>setOpen(false)}>
      <section className="operations-modal" role="dialog" aria-modal="true" aria-label="Central de dados e validação" onMouseDown={(event)=>event.stopPropagation()}>
        ...
        <div className="operations-grid">
          ...
          <article className="operations-card"><h2>JSON de builds</h2><p>Exporta todas as builds com identificação do Pokémon e importa atualizando pelo mesmo nome.</p><div className="card-actions"><button className="secondary-button" disabled={busy} onClick={()=>void runFileOperation(()=>window.gestorPoke.data.exportBuilds(),"Exportação de builds")}>Exportar builds</button><button className="primary-button" disabled={busy} onClick={()=>void runImportOperation(()=>window.gestorPoke.data.importBuilds(),"Importação de builds")}>Importar builds</button></div></article>
          <article className="operations-card"><h2>JSON de equipes</h2><p>Exporta equipes com membros e builds. A importação recria ou atualiza equipes pelo nome.</p><div className="card-actions"><button className="secondary-button" disabled={busy} onClick={()=>void runFileOperation(()=>window.gestorPoke.data.exportTeams(),"Exportação de equipes")}>Exportar equipes</button><button className="primary-button" disabled={busy} onClick={()=>void runImportOperation(()=>window.gestorPoke.data.importTeams(),"Importação de equipes")}>Importar equipes</button></div></article>
        </div>
