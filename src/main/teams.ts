...
function ensureTeamSchema(): void {
  ...
  if (!teamColumns.some((column) => column.name === "regulation_key")) db.exec("ALTER TABLE teams ADD COLUMN regulation_key TEXT NOT NULL DEFAULT 'open'");
  const buildColumns = db.pragma("table_info(builds)") as Array<{ name: string }>;
  if (!buildColumns.some((column) => column.name === "is_primary")) db.exec("ALTER TABLE builds ADD COLUMN is_primary INTEGER NOT NULL DEFAULT 0");
